...
import {
    fn_show_inventory_summary,
    fn_show_inventory_detail,
    fn_token_decode
...
const InventoryTransaction = ({
    ...
    const [searchReport, setsearchReport] = useState({
        ...
        toDate: getTodayDate(),
        productId: null,
        fileName: "InventoryTransaction.csv"
    ...
    const handleDetailInvoice = async (formDate, toDate, productId) => {
        try {
            const result = await fn_show_inventory_detail(formDate, toDate, productId)
    ...
    const handlesearchReportButon = async () => {
        try {
            const resultShowInvoice = await fn_show_inventory_summary(searchReport.fromDate, searchReport.toDate, searchReport.productId);
    ...
    const handleExportReport = async () => {
        try {
            const resultShowInvoice = await fn_show_inventory_summary(
                searchReport.fromDate,
                searchReport.toDate,
                searchReport.productId
            );
            ...
            <div className="card-header bg-primary text-white py-3 px-3 px-sm-4">
                <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div className="lh-sm">
                            <div className="fw-semibold">Inventory Transaction</div>
                    ...
                    >
                        ...
                        >
                            <tr className="text-center">
                                ...
                                <th className="d-none d-sm-table-cell">Product Id</th>
                                <th className="d-none d-sm-table-cell">Product Name</th>
                                <th>Cummulative Quantity</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {allinvoices?.map((inv, idx) => {
                                ...
                                const isExpanded = detailInvoice?.DocEntry === inv.ProductId;
                                        ...
                                        <tr className={`text-center ${isExpanded ? "table-striped" : ""}`}>
                                            <td>
                                                <button
                                                    ...
                                                    onClick={() => handleDetailInvoice(searchReport.fromDate, searchReport.toDate, inv.ProductId)}
                                            ...
                                            <td className="fw-semibold d-none d-sm-table-cell">{inv.ProductId}</td>

                                            <td className="fw-semibold d-none d-sm-table-cell">{inv.ProductName}</td>

                                            <td className="fw-semibold d-none d-sm-table-cell">
                                                {inv.Quantity}
                                            </td>
                                        </tr>
                                                            ...
                                                            <table className="table table-sm table-bordered table-hover mb-0 border-secondary">
                                                                ...
                                                                >
                                                                    <tr>
                                                                        ...
                                                                        <th style={{ minWidth: 110 }}>Document Name</th>
                                                                        <th style={{ minWidth: 220 }}>Document Number</th>
                                                                        <th style={{ minWidth: 220 }}>Transaction Date</th>
                                                                        <th style={{ minWidth: 120 }}>Transaction Type</th>
                                                                ...
                                                                <tbody className="text-center align-middle">
                                                                    {Array.isArray(detailInvoice) && detailInvoice.length > 0 ? (
                                                                        detailInvoice.map((line, lidx) => (
                                                                            <tr key={lidx}>
                                                                                <td></td>
                                                                                <td className="text-nowrap">{line.DocumentName}</td>
                                                                                <td className="text-nowrap">{line.RefernceNumber}</td>
                                                                                <td className="text-nowrap">{line.TransactionDate}</td>
                                                                                <td>{line.TransactionType || "-"}</td>
                                                                                <td>{line.Quantity || "0.00"}</td>
                                                                            </tr>
                                                                        ))
                                                                    ) : (
                                                                        <tr>
...
export default connect(mapStateToProps, mapDispatchToProps)(InventoryTransaction)
